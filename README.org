...
This is sampling from a $Uniform(-1e5, 1e5)$
...
This is a specific version of the [[https://en.wikipedia.org/wiki/Berry%E2%80%93Esseen_theorem#cite_note-1][Berry-Esseen Theorem]], which states that the distribution of the sample means from a specific subset of random variables will converge to the standard normal, by an order of $\frac{1}{n}$.
